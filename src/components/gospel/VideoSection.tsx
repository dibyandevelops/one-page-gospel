...
export function VideoSection({ isKidsMode = false }: VideoSectionProps) {
  ...
  return (
    <section className="rounded-3xl p-6 sm:p-10 bg-gradient-to-b from-gospel-surface via-gospel-surface/90 to-gospel-surface border border-amber-500/30 shadow-[0_0_50px_-15px_rgba(245,158,11,0.2)] my-10 overflow-hidden relative">
            ...
            {!isPlaying ? (
              ...
              >
                ...
                <div className="relative z-10 flex flex-col items-center gap-3">
                  <button
                    className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-tr from-amber-500 to-amber-400 text-slate-950 flex items-center justify-center shadow-[0_0_40px_rgba(245,158,11,0.6)] group-hover:scale-110 transition-all duration-300 ring-4 ring-white/30"
                    aria-label="Play Gospel In One Minute video"
                  >
                    <Play className="w-9 h-9 sm:w-11 sm:h-11 fill-current ml-1" />
                  </button>
                  <span className="text-xs sm:text-sm font-bold text-white bg-black/70 backdrop-blur-md px-3 py-1 rounded-full border border-white/20 shadow-md">
                    Click to Play 60-Second Video
                  </span>
                </div>

                {/* Bottom video tag */}
                <div className="absolute bottom-4 left-5 right-5 flex items-center justify-between text-xs text-white/90 z-10 pointer-events-none">
                  <span className="font-semibold drop-shadow-md text-sm">
